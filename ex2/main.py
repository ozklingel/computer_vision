...
def houghDemo():
    img = cv2.imread('input/pool_balls.jpeg', cv2.IMREAD_GRAYSCALE) / 255
    min_r, max_r = 10, 20
...
def derivDemo():
    ...
    ori = np.arctan2(Y, X).astype(np.float)
    mag = np.sqrt(X ** 2 + Y ** 2).astype(np.float)
